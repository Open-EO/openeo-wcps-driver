...
public class PreviewApi {
	private final PreviewApiService delegate;

	public PreviewApi(@Context ServletConfig servletContext) {
		PreviewApiService delegate = null;

		if (servletContext != null) {
			String implClass = servletContext.getInitParameter("ExecuteApi.implementation");
			if (implClass != null && !"".equals(implClass.trim())) {
				try {
					delegate = (PreviewApiService) Class.forName(implClass).newInstance();
				} catch (Exception e) {
	...
	public Response previewOptions(@Context SecurityContext securityContext) throws NotFoundException {
		return delegate.previewOptions(securityContext);
	}
	...
	public Response previewPost(
		...
		return delegate.previewPost(job, securityContext);
	}
}
